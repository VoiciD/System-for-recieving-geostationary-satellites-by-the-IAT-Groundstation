...
\begin{figure}[H]
    \centering
    \includesvg[width=0.5\linewidth]{Bilder/Skizze Downlink.svg}
    \caption{Vereinfachte Darstellung des Downlinks}
    \label{SkizzeDownlink}
\end{figure}
Die Abbildung \ref{SkizzeDownlink} zeigt eine vereinfachte Darstellung des Downlinks zwischen dem Satelliten Es'Hail-2 (QO-100) und des Empfangssystem der Bodenstation am IAT. Der Downlink kann in drei Bereiche eingeteilt werden - Dem Sender, der Übertragungsstrecke und dem Empfänger. 
...
\begin{equation}
    EIRP=10^{\frac{59.5\,\mathrm{dBm}}{10}}\cdot0.001\,\text{W}=891.251\,\text{W}
...
\begin{figure}[H]
    \centering
    \includesvg[width=0.4\linewidth]{Bilder/Freiraumdämpfung Beispiel.svg}
...
\begin{equation}
    ...
    \label{eq:Berechnung-Fehlausrichtung-Sender}
\end{equation}
...
\begin{equation}
    ...
    \label{eq:Berechnung-Fehlausrichtung-Empfänger}
\end{equation}
...
\begin{equation}
    ...
    \label{eq:Winkel-der-Fehlausrichtung-Sender}
\end{equation}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Bilder/Impedanzkompromiss.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{Bilder/Antenne mit Feed.jpg}
    \caption{Vorhandene Parabolantenne mit neuem Antennenfeed.}
    \label{fig:vorhandene-Parabolantenne}
\end{figure}

Die Bild \ref{fig:vorhandene-Parabolantenne} zeigt die auf dem Dach der Hochschule vorhandene Parabolantenne. Beim Parabolspiegel handelt es sich um eine Kathrein CAS 90 HD ohne Logo. Der Parabolspiegel hat einen Durchmesser von $d_\mathrm{Antenne}=0.9\,\text{m}$ und ist für den Betrieb im X-Band und unteres Ku-Band, genauer zwischen $10.7\,\text{GHz}$ und $12.75\,\text{GHz}$, vorgesehen.\cite{KathreinCAS90HD}\newline
...
\end{equation}
Der bisher montierte LNB ist für den Empfang von Satellitenfernsehen ausgelegt und für die gewünschte Anwendung nicht geeignet. Daher wird dieser gegen einen Triplebandfeed von der Firma BaMaTech ausgetrauscht. Dieser Antennenfeed ist speziell für den Einsatz an Es'Hail-2 (QO-100) entwickelt und kann in drei verschiedenen Frequenzbänder eingesetzt werden. Daher kommt auch der folgt auch sein Name.\cite{amatech-feed}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Bilder/Triplebandfeed.jpg}
    \caption{Das Bild zeigt den ausgewählten Triplebandfeed von BaMaTech. Die schwarze Halterung am Antennenfeed dient zur Montage des Antennenfeeds in einem Halter. Diese ist im Zuge der Arbeit entwickelt worden.}
    \label{fig:verwendeter Triplebandfeed}
\end{figure}
Das Abbildung \ref{fig:verwendeter Triplebandfeed} zeigt den Triplebandfeed von der Firma BaMaTech. Im Grunde handelt es sich dabei um eine Hornantenne mit drei Anschlüssen für die einzelnen Frequenzbänder. Verwendet werden kann der Triplebandfeed im S-Band bei $2.4\,\text{GHz}$, im C-Band bei $5.6\,\text{GHz}$ und im X-Band bei $10.5\,\text{GHz}$, wobei im Zuge dieser Arbeit nur der Anschluss für das X-Band, wo sich der Downlink von Es'Hail-2 (QO-100) befindet, interessant.\cite{amatech-feed}\newline 
Angeschlossen werden kann der Triplebandfeed mittels SMA-Steckverbindungen.\cite{amatech-feed}\newline
Um die Tauglichkeit der Parabolantenne und des Feed für die Anwendung an Es'Hail-2 (QO-100) zu überprüfen kann zu einem die Effizienz $\eta_\mathrm{ANT}$ der effektive Antennenfläche $A_\mathrm{E}$ bestimmt werden. Auch kann die Reflexion $S11$ des Antennenfeeds gemessen und anschließend das $VSWR$ werden.\newline
...
\subsubsection*{Rauscharmer Signalumsetzer und Fernspeiseweiche}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Bilder/MKU LNC10 QO 100.jpg}
    \caption{Der vewendete MKU LNC 10 QO-100 von Kuhne Electronic. Die 2-Euro Münze dient zum Größenvergleich.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Bilder/KU BT 10 REF.jpg}
    \caption{Die verwendete Fernspeiseweiche KU BT 10 REF von Kuhne Electronic. Die 2-Euro Münze dient zum Größen Vergleich.}
...
Das Herzstück des USRP X310 bilden die beiden die beiden modularen Erweiterungskarten. Insgesamt stehen 10 verschiedene Karten zur Auswahl. Von Ihnen ist der Frequenzbereich, die maximale Bandbreite und die Verstärkung abhängig. In dem USRP X310 sind zwei UBX Erweiterungskarten verbaut. Die UBX Erweiterungskarte erlaubt es dem USRP X310 in einem Frequenzbereich von $10\,\text{MHz}$ bis $6\,\text{GHz}$ zu arbeiten. Die maximale Bandbreite beträgt dabei $160\,\text{MHz}$. Die maximale Verstärkung der UBX Erweiterungskarte beträgt $G_\mathrm{SDR}=31.5\,\text{dB}$\cite{USRP-X310}\cite{USRP-X310-UBX-Doku}\newline
...
Im Empfangssystem sind nur zwei Verstärkenden Komponenten vorhanden. Die erste Komponente ist der LNC, welcher mit einer typischen Verstärkung von $G_\mathrm{LNC}=55\,\text{dB}$ angeben wird. Die zweite Komponente ist der USRP X310, welcher eine variable Verstärkung besitzt. Für Rechenzwecke wird hier von einer Verstärkung von $G_\mathrm{SDR}=31.5\,\text{dB}$ angegeben. Diese beiden Verstärkungen können in eine Größe $G$ zusammengefasst werden.
\begin{equation*}
    G=G_\mathrm{LNC,dB}+G_\mathrm{SDR,dB}=55\,\text{dB}+31.5\,\text{dB}=86.5\,\text{dB}
\end{equation*}
...
\begin{equation}
    G_\mathrm{sys}=G_\mathrm{LNC,dB}-L_\mathrm{sys,dB}=86.5\,\text{dB}-7.01\,\text{dB}=79.49\,\text{dB}
    \label{eq:Gesamtverstärkung-Empfangssystems}
\end{equation}
Die Gesamtverstärkung des Empfangssystems beträgt maximal $G_\mathrm{sys}=79.49\,\text{dB}$. Mit dieser Verstärkung ist es problemlos möglich die schwache Signale von Es'Hail-2 (QO-100) zu verstärken. 
...
\begin{figure}[H]
    ...
    \caption{Grafische Darstellung der Einflussfaktoren auf das Link Budgets}
...
\begin{split}
        ...
        &=891.25\,\text{W}\cdot 7244.36\cdot88.98\cdot10^{6}\cdot\frac{1}{2.9\cdot10^{20}}\cdot\frac{1}{3.33}\cdot\frac{1}{0.69}\cdot\frac{1}{1.13}\\
        &=7.63\cdot 10^{-7}\,\text{W} =-31.18\,\text{dBm}
\end{split}
    \label{eq:Ausgang-Leistung-klarer-Himmel}
\end{equation}
Durch die Verstärkung des Eingangssignals um $79.49\,\text{dB}$ beträgt die Leistung am Ausgang des RF-Bereiches $P_\mathrm{RX}=-31.18\,\text{dBm}$. Der Pegel des Signals liegt deutlich über dem Grundrauschen des SDR. Allerdings kommt durch das restlichen Empfangssystem noch zusätzliches Rauschen in den SDR, weshalb noch keine Aussage auf die mögliche Verarbeitung des Signals getroffen werden kann. Dafür muss noch das $SNR$ bestimmt werden.\newline
...
\begin{table}[H]
    ...
    Verstärkung des Empfangssystem  & $G_\mathrm{sys}$        & $79.49$ & $\text{dB}$\\
                            & $G_\mathrm{sys}$      & $88.98\cdot 10^{6}$ \\
    Leistung am Ausgang     & $P_\mathrm{RX}$        & $7.63\cdot 10^{-7}$  & $\text{W}$\\
                            & $P_\mathrm{RX}$        & $-31.18$        & $\text{dBm}$\\
...
\begin{split}
        ...
        &=891.25\,\text{W}\cdot 7244.36\cdot88.98\cdot10^{6}\cdot\frac{1}{2.9\cdot10^{20}}\cdot\frac{1}{3.33}\cdot\frac{1}{0.69}\cdot\frac{1}{1.24}\\
        &=6.95\cdot 10^{-7}\,\text{W} =-31.58\,\text{dBm}
\end{split}
    \label{eq:Ausgang-Leistung-leichter-Regen}
\end{equation}
Nach der Verstärkung beträgt die Leistung am Ausgang des RF-Bereiches $P_\mathrm{RX}=-31.58\,\text{dBm}$. Der Verlust gegenüber der Ausgangsleistung bei klaren Himmel $P_\mathrm{RX}=-31.18\,\text{dBm}$ beträgt $0.4\,\text{dBm}$ oder $8.8\,\%$.\newline
...
\begin{equation}
C/N_\mathrm{o}=\frac{P_\mathrm{RX}}{k\cdot T_\mathrm{S}}=\frac{6.95\cdot 10^{-7}\,\text{W}}{1.38\cdot10^{-23}\,\frac{\text{J}}{\text{K}}\cdot572.71\,\text{K}}=8.79\cdot10^{13}\,\text{Hz}=139.44\,\text{dBHz}
...
\begin{table}[H]
    ...
    Verstärkung des Empfangssystem  & $G_\mathrm{sys}$        & $79.49$ & $\text{dB}$\\
                            & $G_\mathrm{sys}$      & $88.98\cdot 10^{6}$ \\
    Leistung am Ausgang     & $P_\mathrm{RX}$        & $6.95\cdot 10^{-7}$  & $\text{W}$\\
                            & $P_\mathrm{RX}$        & $-31.58$        & $\text{dBm}$\\
    Äquivalente Rauschzahl  & $T_\mathrm{e,sys}$    & $336.63$          & $\text{K}$ \\
    Bei $B=500\,\text{kHz}$ & & & \\
...
Der Pegel $P_\mathrm{RX}$ des Signals am Ausgang des RF-Bereiches kann mit der Gleichung \ref{eq:Ausgang-Leistung} bestimmt werden. Die Verstärkung des Systems ist in Gleichung \ref{eq:Gesamtverstärkung-Empfangssystems} angegeben und beträgt $G_\mathrm{sys}=79.49\,\text{dB}=88.98\cdot 10^{6}$.
\begin{equation}
\begin{split}
        ...
        &=891.25\,\text{W}\cdot 7244.36\cdot88.98\cdot 10^{6}\cdot\frac{1}{2.9\cdot10^{20}}\cdot\frac{1}{3.33}\cdot\frac{1}{0.69}\cdot\frac{1}{9.14}\\
        &=9.43\cdot 10^{-8}\,\text{W} =-40.25\,\text{dBm}
\end{split}
    \label{eq:Ausgang-Leistung-Regen}
\end{equation}
Auch nach der Verstärkung des sehr schwachen Signals bleibt der Pegel mit $P_\mathrm{RX}=-40.25\,\text{dBm}$ niedrig. Bei einem klaren Himmel kann eine Ausgang des RF-Bereiches eine Leistung von $P_\mathrm{RX}=-31.18\,\text{dBm}$, bzw. $P_\mathrm{RX}=-31.58\,\text{dBm}$ bei leichten Regenschauern, erreicht.\newline
...
\begin{equation}
C/N_\mathrm{o}=\frac{P_\mathrm{RX}}{k\cdot T_\mathrm{S}}=\frac{9.43\cdot 10^{-8}\,\text{W}}{1.38\cdot10^{-23}\,\frac{\text{J}}{\text{K}}\cdot616.69\,\text{K}}=11.08\cdot10^{12}\,\text{Hz}=130.45\,\text{dBHz}
 \label{eq:Qualität-Downlink-Regen}
\end{equation}
Im Vergleich zur Qualität des Downlinks bei einem klaren Himmel $C/N_\mathrm{o}=139.87\,\text{dBHz}$, wird die Qualität bei starken Niederschlägen um $9.42\,\text{dB}$ auf $C/N_\mathrm{o}=130.45\,\text{dBHz}$ gesenkt. Das entspricht einer Verschlechterung der Qualität um $88.4\,\%$. Verglichen zur Qualität bei leichten Regenschauern $C/N_\mathrm{o}=139.44\,\text{dBHz}$ verschlechtert sich die Qualität um $87.2\,\%$.
\begin{table}[H]
    \centering
    \begin{tabular}{c|c|c|c}
        Name & Variable & Wert & Einheit\\
    ...
    Verstärkung des Empfangssystem  & $G_\mathrm{sys}$        & $79.49$ & $\text{dB}$\\
                            & $G_\mathrm{sys}$      & $88.98\cdot 10^{6}$ \\
    Leistung am Ausgang     & $P_\mathrm{RX}$        & $9.43\cdot 10^{-8}$  & $\text{W}$\\
                            & $P_\mathrm{RX}$        & $-40.25$        & $\text{dBm}$\\
    Äquivalente Rauschzahl  & $T_\mathrm{e,sys}$    & $336.63$          & $\text{K}$ \\
    Bei $B=500\,\text{kHz}$ & & &\\
    ...
    Link Qualität           & $C/N_\mathrm{o}$ & $130.45$ & \text{dB/Hz} \\
    \end{tabular}
