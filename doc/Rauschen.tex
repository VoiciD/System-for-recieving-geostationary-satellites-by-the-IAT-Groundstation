...
\begin{figure}[H]
    \centering
    \includesvg[width=0.7\linewidth]{Bilder/Kettenschaltung Zweitore}
    \caption{Kaskadierung von mehreren Zweitoren}
    \label{fig:Kettenschaltung-Zweitore}
\end{figure}
Beim Kaskadieren von mehreren Zweitoren, wie in Abbildung \ref{fig:Kettenschaltung-Zweitore} dargestellt, setzt sich die Gesamtrauschzahl $F_\mathrm{ges}$ aus der Summe der Rauschzahlen $F_\mathrm{x}$, wobei $x=1,2,3,\dots$, der einzelnen Zweitore, sowie deren Verstärkungen $G_\mathrm{x}$, zusammen.\cite{HEUERMANN_2018}
\begin{equation}
